...
var
  mono_images_init : procedure;
  mono_images_cleanup : procedure;
  mono_image_open : function(fname:Pchar; status:PMonoImageOpenStatus):PMonoImage;
  mono_image_open_full : function(fname:Pchar; status:PMonoImageOpenStatus; refonly:mono_bool):PMonoImage;
  mono_pe_file_open : function(fname:Pchar; status:PMonoImageOpenStatus):PMonoImage;
  mono_image_open_from_data : function(data:Pchar; data_len:uint32_t; need_copy:mono_bool; status:PMonoImageOpenStatus):PMonoImage;
  mono_image_open_from_data_full : function(data:Pchar; data_len:uint32_t; need_copy:mono_bool; status:PMonoImageOpenStatus; refonly:mono_bool):PMonoImage;
  ...
  name:Pchar):PMonoImage;
  mono_image_fixup_vtable : procedure(image:PMonoImage);
  mono_image_loaded : function(name:Pchar):PMonoImage;
  mono_image_loaded_full : function(name:Pchar; refonly:mono_bool):PMonoImage;
  mono_image_loaded_by_guid : function(guid:Pchar):PMonoImage;
  mono_image_loaded_by_guid_full : function(guid:Pchar; refonly:mono_bool):PMonoImage;
  mono_image_init : procedure(image:PMonoImage);
  mono_image_close : procedure(image:PMonoImage);
  mono_image_addref : procedure(image:PMonoImage);
  mono_image_strerror : function(status:MonoImageOpenStatus):Pchar;
  mono_image_ensure_section : function(image:PMonoImage; section:Pchar):longint;
  mono_image_ensure_section_idx : function(image:PMonoImage; section:longint):longint;
  mono_image_get_entry_point : function(image:PMonoImage):uint32_t;
  mono_image_get_resource : function(image:PMonoImage; offset:uint32_t; size:Puint32_t):Pchar;
  mono_image_load_file_for_image : function(image:PMonoImage; fileidx:longint):PMonoImage;
  mono_image_load_module : function(image:PMonoImage; idx:longint):PMonoImage;
  mono_image_get_name : function(image:PMonoImage):Pchar;
  mono_image_get_filename : function(image:PMonoImage):Pchar;
  mono_image_get_guid : function(image:PMonoImage):Pchar;
  mono_image_get_assembly : function(image:PMonoImage):PMonoAssembly;
  mono_image_is_dynamic : function(image:PMonoImage):mono_bool;
  mono_image_rva_map : function(image:PMonoImage; rva:uint32_t):Pchar;
  mono_image_get_table_info : function(image:PMonoImage; table_id:longint):PMonoTableInfo;
  mono_image_get_table_rows : function(image:PMonoImage; table_id:longint):longint;
  mono_table_info_get_rows : function(table:PMonoTableInfo):longint;
  { This actually returns a MonoPEResourceDataEntry *, but declaring it
      * causes an include file loop. }
  mono_image_lookup_resource : function(image:PMonoImage; res_id:uint32_t; lang_id:uint32_t; name:Pmono_unichar2):pointer;
  mono_image_get_public_key : function(image:PMonoImage; size:Puint32_t):Pchar;
  mono_image_get_strong_name : function(image:PMonoImage; size:Puint32_t):Pchar;
  mono_image_strong_name_position : function(image:PMonoImage; size:Puint32_t):uint32_t;
  mono_image_add_to_name_cache : procedure(image:PMonoImage; nspace:Pchar; name:Pchar; idx:uint32_t);
  mono_image_has_authenticode_entry : function(image:PMonoImage):mono_bool;
